...
export async function GET(
  ...
  try {
    if (!params.bannerId) {
      return new NextResponse("Banner id not found", { status: 400 });
    }
...
export async function PATCH(
  ...
  try {
    ...
    if (!label) {
      return new NextResponse("Label harus diinput", { status: 400 });
    }
    if (!imageUrl) {
      return new NextResponse("ImageUrl harus diinput", { status: 400 });
    }
